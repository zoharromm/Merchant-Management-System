    ...
    public partial class Form1 : Form
    {
                ...
                foreach (HtmlNode prd in productCollection)
                {
                    ...
                    if (_CollectionCatLink != null)
                    {
                        foreach (HtmlNode node in _CollectionCatLink)
                        {
                            ...
                            {
                                if (attr.Name == "href")
                                    if (!Producturl.Keys.Contains("http://www.bestbuy.ca/" + attr.Value))
                                    {
                                        ...
                                        if (_CollectionStock != null)
                                        {
                                            ...
                                            if (sku != "")
                                            {
                                                try
                                                {
                                                    Producturl.Add("http://www.bestbuy.ca/" + attr.Value, sku);
                                                    skus = skus + sku + "|";
                                                    #region Price
                                                    try
                                                    {
        ...
        public void GetProductInfo(HtmlAgilityPack.HtmlDocument _doc, string url)
        {
            ...
            try
            {
                ...
                #endregion UPC
                product.Name = product.Name.Replace("- Only at Best Buy", "");
                product.Name = product.Name.Replace("Best Buy", "");
                product.Description = product.Description.Replace("- Only at Best Buy", "");
                product.Description = product.Description.Replace("Best Buy", "");
        ...
        public bool GetOnlineStock(string skus, List<BusinessLayer.InventorySync> PrdData)
        {
            ...
            try
            {
                HttpWebRequest Request = (HttpWebRequest)WebRequest.Create("http://api.bestbuy.ca/availability/products?accept-language=en-CA&skus=" + skus + "&accept=application%2Fvnd.bestbuy.simpleproduct.v1%2Bjson");
        ...
        public void work_dowork(object sender, DoWorkEventArgs e)
        {
            ...
            else if (_IsProduct && !_Iserror)
            {
                ...
                { WriteLogEvent(Url1, "Issue accured in reading product Info." + exp.Message); }
